...
5. **Reconstitution calculator** — **type-to-search any compound** (autocomplete dropdown loads its vial mg + a typical research dose); mg + BAC water + dose → U-100 units, volume/dose, concentration, doses/vial; overfill warning; **2 mL BAC water is the recommended default**. Includes a **session log**: add multiple compounds to a table (compound, reconstitution recipe, draw, volume, doses) with per-row remove + clear all.
