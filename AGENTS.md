...
- **Convex has to actually deploy, and CI is the only thing that checks** — nothing else looks at `convex/`: vitest resolves it through Vite, `tsc` has no opinion on runtimes, `next build` ignores the directory. Two rules, both enforced by `tests/convex-deployability.test.ts`. **Module paths must be valid identifiers** — Convex turns each one into an `api.d.ts` identifier and a dotted function reference, so `logic-push.ts` is undeployable however well it tests; that is why the pure modules are `logicPush.ts`, not `logic-push.ts`. **`"use node"` must stay unreachable from the default runtime** — a *value* import of a Node-runtime module, at any depth, bundles its Node dependencies into the default runtime and fails the deploy, so share constants through a pure module and invoke actions through the generated `api`. Type-only imports are erased and are fine. `api.d.ts` is generated: run `npx convex codegen`, never hand-edit it.
