...
for (const [label, viewport] of [
  ...
  await check("quick actions open craving sheet", async () => {
    await page.evaluate(() => document.querySelector("nextjs-portal")?.remove());
    await page.locator('nav[aria-label="Sections"] button', { hasText: /^Today$/ }).click({ force: true });
    await page.getByRole("button", { name: "Quick log craving" }).click({ force: true });
    await page.getByRole("dialog", { name: /log craving/i }).waitFor();
